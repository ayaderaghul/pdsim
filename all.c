...
void regenerate(Agent pop[POP_SIZE])
{
    ...
    for (int i = 0; i < POP_SIZE; i++)
    {
        if (pop[i].accumAvgPayoff < 0.0f)
            pop[i].accumAvgPayoff = 0.0f; // enforce non-negative fitness
    ...
    for (int k = 0; k < POP_SIZE; k++)
    {
        float r = randFloat();
        int chosen = 0;
        while (chosen < POP_SIZE && r > cumulative[chosen])
            chosen++;
        if (chosen >= POP_SIZE)
            chosen = POP_SIZE - 1;
...
void mutate(Agent pop[POP_SIZE])
{
    ...
    if (numMutations < 1)
        numMutations = 1; // at least one mutation
